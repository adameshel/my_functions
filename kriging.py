...
def krige( P, u, N, covfct, Kinverse, pykrige_pars=None ):
    ...
    if pykrige_pars:
        k = (pykrige_pars[0] + pykrige_pars[0] * 0.1) - \
...
#                        import pdb; pdb.set_trace()
    ...
    #             import pdb; pdb.set_trace()
            # noise_in_function = False
            if optimize is True:
                cov_func = lambda h: (Sill + Sill * 0.1) - func_to_opt( h, Sill, Range )
            else:
                cov_func = lambda h: (Sill + Sill * 0.1) - func_with_nugget( h, Sill, Range, nugget )
...
#        import pdb; pdb.set_trace()
        if pykrige_opt_pars is False:
            ...
            K = (Sill + Sill * 0.1) - temp_variog.variogram_function([Sill,Range], K)
        K = np.array( K )
        K = K.reshape( N,N )
        ...
        if (K_matCond > cond_num_thresh) and pykrige_opt_pars is False:
            ...
            cov_func = lambda h: (Sill + Sill * 0.1) - func_with_noise( h, Sill, Range )
